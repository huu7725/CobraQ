...
def is_red_text(color_int):
    """
    Nhận dạng chữ màu đỏ (PyMuPDF lưu màu dạng integer 0xRRGGBB).
    File dạng: đáp án đúng được tô chữ đỏ.
    """
...
def parse_pdf_inline(page, hl_rects):
    ...
    Hỗ trợ 2 loại đánh dấu đáp án:
      - Màu nền (background fill) → span_in_rect
      - Màu chữ đỏ (text color)  → is_red_text
    """
    lines = []
    try:
        for block in page.get_text("dict")["blocks"]:
            ...
            for line in block["lines"]:
                line_text = ""
                hl_bg   = False  # Tô nền màu
                hl_red  = False  # Chữ màu đỏ
                for sp in line["spans"]:
                    line_text += sp["text"]
                    # Kiểm tra màu nền
                ...
                if t:
                    lines.append({
                        "text": t,
                        "hl": hl_bg or hl_red,   # Bất kỳ cách nào cũng tính
                        "hl_bg": hl_bg,
                        "hl_red": hl_red
                    })
    except Exception as e:
...
def parse_pdf(content: bytes) -> list:
    """
    Parser PDF chính — hỗ trợ 3 dạng đáp án:
    1. Bảng + tô màu nền ô (cyan/xanh/vàng)
    2. Inline + tô màu nền span
    3. Inline + chữ màu đỏ  ← file sinh học của bạn
    """
    import fitz
    ...
    for page in doc:
        ...
        # Thử parse dạng bảng trước (chỉ khi có ô màu nền)
        if hl_rects:
            try:
        ...
        # Dạng inline (chữ đỏ hoặc nền màu span)
        # → luôn chạy kể cả khi không có hl_rects
        try:
